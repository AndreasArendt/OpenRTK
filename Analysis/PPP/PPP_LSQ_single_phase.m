...
idx_Galileo = cellfun(@(x)strcmp(x(1), 'E'), S.SvSystem_t_eph).*0;
...
for tt = timestamps.'   
    ...
    code_2_idx = code_idx & (S.Band == 2);
    
    phase_idx   = idx & (idx_Galileo | idx_GPS) & ...
                       (S.Phase_1 > 0) & (S.Code_2 > 0) & ...                   % valid Carrierphases
                       ((S.Band == 1) | (S.Band == 2));                         % E1 and E5
    phase_1_idx = phase_idx & (S.Band == 1);
    phase_2_idx = phase_idx & (S.Band == 2);

    [ia,ib] = ismember(S.SvSystemID_t_eph(code_1_idx), S.SvSystemID_t_eph(code_2_idx));
    tmp = find(code_1_idx);
    code_1_idx(tmp(~ia)) = 0;
    
    [ia,ib] = ismember(S.SvSystemID_t_eph(phase_1_idx), S.SvSystemID_t_eph(phase_2_idx));
    ...
    for ii = 1:5
        ...
        % Iono-Free LC
        rho_iono_free = Generic.CalcIonoFreeLinearCombination(S.Code_1(code_1_idx), S.Code_2(code_2_idx), F_E1_Galileo__Hz, F_E5a_Galileo__Hz);
        phi_iono_free = Generic.CalcIonoFreeLinearCombination(S.Phase_1(phase_1_idx), S.Phase_2(phase_2_idx), F_E1_Galileo__Hz, F_E5a_Galileo__Hz);
