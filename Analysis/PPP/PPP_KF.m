...
nAmbiguity = numel(unique(S.SvSystemID_t_eph(idx_SV)));

% KF Initialization
nStates = 8 + nAmbiguity;
F = eye(nStates);
F(1:3, 4:6) = eye(3) .* 30; % I * dt

Q_pos = [0.01 0.01 0.01];
Q_vel = [0.01 0.01 0.01];
Q_cdt = 1e-3;
Q_ztd = 1e-9;
Q_N   = 0.01 .* ones(1, nAmbiguity); % Ambiguities

Q = diag([Q_pos, Q_vel, Q_cdt, Q_ztd, Q_N]);
kf = Kalman.Kalman(F, Q);
kf.SetInitialStates([station_pos__m'; zeros(3,1); 0; 0; zeros(nAmbiguity,1)]);    % ECEF Position [m]
                                                                                  % ECEF Velocity [m]
...
for tt = timestamps.'   
    ...
    phase_idx   = idx & (idx_Galileo | idx_GPS) & ...
                       ...
                       ((S.Band == 1) | (S.Band == 5));                         % E1 and E5
    phase_1_idx = code_idx & (S.Band == 1);
    phase_5_idx = code_idx & (S.Band == 5);
    ...
    for ii = 1:KfIterations
        ...
        nObs = numel(e(:,1));
        H = [-e, zeros(nObs, 3), ones(nObs,1), M_E, zeros(nObs, nAmbiguity)];
